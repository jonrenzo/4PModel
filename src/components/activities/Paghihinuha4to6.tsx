...
export default function Paghihinuha4to6({ rangeId }: { rangeId: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (user) {
        const { data } = await supabase.from("4p_answers").select("*").eq("user_id", user.id).eq("activity_type", "paghihinuha").eq("chapter_range", rangeId);
        if (data) {
          const map: any = {};
          data.forEach((i: any) => {
  ...
  return (
    <div className="pb-4">
      <div className="flex items-center gap-2 mb-3">
        <div className="rounded-full bg-[#3e2723] p-1.5"><svg className="w-4 h-4 text-[#d4af37]" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clipRule="evenodd" /></svg></div>
        <h2 className="font-serif text-lg font-bold text-[#3e2723]">Paghihinuha</h2>
      </div>
      <p className="mb-3 text-xs text-[#5d4037]">Suriin ang bawat bagay at tukuyin ang tauhan at pahiwatig.</p>

      <div className="border rounded-xl overflow-hidden">
        <div className="flex bg-[#d7ccc8] text-xs font-bold">
          <div className="flex-1 p-2 border-r border-[#d7ccc8] text-center">BAGAY</div>
          <div className="flex-1 p-2 border-r border-[#d7ccc8] text-center">TAUHAN</div>
        ...
        {rows.map((row) => (
          <div key={row.id} className="flex text-xs border-t border-[#d7ccc8]">
            <div className="flex-1 p-2 border-r border-[#d7ccc8] flex items-center justify-center gap-1">
              {row.images.map((img, i) => <img key={i} src={img} className="w-8 h-8 object-contain" alt="" />)}
            </div>
            <div className="flex-1 p-1 border-r border-[#d7ccc8]">
              <input className="w-full p-1 text-xs border rounded" value={answers[row.id]?.tauhan || ""} onChange={(e) => setAnswers((p) => ({ ...p, [row.id]: { ...p[row.id], tauhan: e.target.value } }))} />
            </div>
            <div className="flex-1 p-1">
              <input className="w-full p-1 text-xs border rounded" value={answers[row.id]?.pahiwatig || ""} onChange={(e) => setAnswers((p) => ({ ...p, [row.id]: { ...p[row.id], pahiwatig: e.target.value } }))} />
            </div>
          </div>
        ))}
      </div>
