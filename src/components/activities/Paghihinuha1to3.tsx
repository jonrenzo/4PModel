...
export default function Paghihinuha1to3({ rangeId }: { rangeId: string }) {
  ...
  const [clawedCharacter, setClawedCharacter] = useState<Character | null>(
    null,
  );
        ...
        await supabase.from("4p_answers").upsert(
          {
            ...
            question_index: clawedCharacter.id,
    ...
    try {
      ...
      if (answers) {
        ...
        answers.forEach((item) => {
          answersMap[item.question_index] = item.answer;
        });
  ...
  const handleSaveAnswer = async () => {
    ...
    try {
      ...
      setAvailableCharacters((prev) =>
        prev.filter((c) => c.id !== clawedCharacter!.id),
      );
      setCurrentAnswer("");
      setShowQuestionModal(false);
      setClawedCharacter(null);
      alert(
        completedCount + 1 === characters.length
          ? "Mahusay! Tapos na!"
          : "Nai-save na!",
      );
    } catch (error) {
    ...
    <div className="pb-4">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="rounded-full bg-[#3e2723] px-3 py-1">
          <span className="text-sm font-bold text-[#e8d4b0]">
      ...
      <div className="rounded-2xl bg-linear-to-b from-[#8d6e63] to-[#6d4c41] p-3 shadow-lg">
        <div
          className="rounded-xl border-2 border-[#d4af37] bg-linear-to-b from-[#e3f2fd] to-[#bbdefb] relative overflow-hidden"
          style={{ height: 200 }}
        >
          {/* Claw */}
          <motion.div
          ...
          >
            ...
            <div className="w-12 h-12 rounded-full bg-linear-to-b from-gray-400 to-gray-600 flex items-center justify-center mx-auto">
          ...
          <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-2 px-2 flex-wrap">
            {availableCharacters.slice(0, 6).map((char) => (
              <div
                key={char.id}
                className="w-10 h-10 rounded-full border-2 border-white shadow overflow-hidden bg-gray-200"
              >
                <img
                  src={char.image}
                  alt={char.name}
          ...
          >
            ...
            >
              <div className="bg-linear-to-b from-[#f5e6d3] to-white p-4 text-center">
                <img
                  src={clawedCharacter.image}
                  alt={clawedCharacter.name}
                  className="w-20 h-20 rounded-full border-4 border-[#d4af37] mx-auto object-cover"
                />
                <h3 className="mt-2 font-serif text-lg font-bold text-[#3e2723]">
              ...
              <div className="p-4">
                ...
                <textarea
                  className="w-full min-h-[80px] rounded-xl border border-[#c4b09a] bg-[#efede6] p-3 text-xs text-[#3e2723] focus:outline-none focus:border-[#8d6e63]"
                ...
                <div className="flex gap-2 mt-3 items-center">
                  ...
                  <button
                    ...
                    className="flex-1 py-2 rounded-full border border-[#8d6e63] text-sm font-bold text-[#5d4037] hover:bg-[#d7ccc8] transition-colors"
                  >
                    Laktawan
                  </button>
                  <button
