    ...
    <div className="pb-4">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="rounded-full bg-[#3e2723] px-3 py-1">
          <span className="text-sm font-bold text-white">{completedKeys.length}/{questions.length}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 mb-4">
        {questions.map((q, i) => (
          <button
            key={q.id}
            onClick={() => { setSelectedKey(q.id); setCurrentAnswer(savedAnswers[q.id] || ""); }}
      ...
      {selectedKey && (
        ...
        >
          <p className="mb-1 text-xs font-bold text-[#3e2723]">Tanong {selectedKey}:</p>
          <p className="mb-3 text-xs text-[#5d4037] leading-relaxed">{questions.find(q => q.id === selectedKey)?.question}</p>
          <textarea
